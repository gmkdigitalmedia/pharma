"""
Script to push the templates directory to GitHub
"""
import os
import subprocess
import sys
...
def push_templates():
    ...
    if not source_path.exists():
        print(f"Templates directory not found: {source_path}")
        return False
        
    dest_path = temp_dir / 'templates'
    
    try:
        if dest_path.exists():
            shutil.rmtree(dest_path)
        print(f"Copying templates directory from {source_path} to {dest_path}")
        shutil.copytree(source_path, dest_path)
    except Exception as e:
        print(f"Error copying templates directory: {e}")
        return False
    
    # Add, commit, and push changes
    if not run_cmd("git add templates/", cwd=temp_dir):
        return False
    
    # Commit with a meaningful message
    if not run_cmd("git commit -m 'Update templates from Replit'", cwd=temp_dir):
    ...
    print("Templates pushed successfully!")
    return True

if __name__ == "__main__":
    success = push_templates()
