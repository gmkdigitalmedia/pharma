...
def register_routes(app):
    ...
    def register_organization():
        ...
        if form.validate_on_submit():
            ...
            flash('Organization registered successfully! You can now log in as an admin.', 'success')
            return redirect(url_for('login'))
            
        return render_template('login.html', form=form, org_register=True)
    ...
    def register():
        ...
        if form.validate_on_submit():
            # Create the user with role and organization from invitation
            user = User(
                username=form.username.data, 
                email=form.email.data,
            ...
            flash(f'Your account has been created as a {invitation.role}! You can now log in.', 'success')
            return redirect(url_for('login'))
            
        return render_template('login.html', form=form, register=True, invitation_token=token)
    ...
    def meditag():
            ...
            if file:
                ...
                try:
                    ...
                    for _, row in tagged_data.iterrows():
                        hcp = HCP(
                            user_id=current_user.id,  # Track which user created this HCP
                            organization_id=current_user.organization_id,  # Link to the organization
                            hcp_id=row['hcp_id'],
        ...
        # Get existing HCPs for current organization (not just user's HCPs)
    ...
    @login_required
    def contentcraft():
        # Get HCPs for the current organization
        hcps = HCP.query.filter_by(organization_id=current_user.organization_id).all()
        ...
        if request.method == 'POST':
            ...
            # Ensure HCP belongs to the user's organization
            if hcp.organization_id != current_user.organization_id:
                flash('Access denied. You can only generate content for HCPs in your organization.', 'danger')
    ...
    @login_required
    def engageoptic():
        # Get contents for the current organization
        contents = Content.query.filter_by(organization_id=current_user.organization_id).all()
        ...
        # Get HCPs for populating dropdown (from organization)
        hcps = HCP.query.filter_by(organization_id=current_user.organization_id).all()
        form.hcp.choices = [(str(hcp.id), f"{hcp.name} ({hcp.tag})") for hcp in hcps]
        
        # Get content for populating dropdown (from organization)
        ...
        if form.validate_on_submit():
            ...
            # Security check - verify these belong to the user's organization
            if hcp.organization_id != current_user.organization_id or content.organization_id != current_user.organization_id:
                flash('Access denied. You can only create campaigns for content and HCPs in your organization.', 'danger')
    ...
    def api_link_asset_to_content(asset_id):
        ...
        # Check if asset and content exist and belong to user
    ...
    def admin_dashboard():
        ...
        active_api_count = ApiConnection.query.filter_by(is_active=True).count()
        inactive_api_count = ApiConnection.query.filter_by(is_active=False).count()
        
        # Check connections to external services
        veeva_connected = ApiConnection.query.filter_by(name='VEEVA', is_active=True).first() is not None
        salesforce_connected = ApiConnection.query.filter_by(name='Salesforce', is_active=True).first() is not None
    ...
    def admin_user_management():
        ...
        # Get pending invitations from this organization
    ...
    def admin_invite_user():
        ...
        if form.validate_on_submit():
            ...
            invitation = Invitation(
                ...
                organization_id=current_user.organization_id,  # Include organization context
            ...
            flash(f'Invitation sent to {form.email.data}! URL: {invite_url}', 'success')
        else:
    ...
    def admin_resend_invitation():
        ...
        flash(f'Invitation resent to {invitation.email}! URL: {invite_url}', 'success')
        return redirect(url_for('admin_user_management'))
    ...
    def accept_invitation(token):
        ...
        if invitation_valid and form.validate_on_submit():
            ...
            user = User(
                ...
                role=invitation.role,
                invited_by=invitation.sender_id,
                organization_id=invitation.organization_id  # Set organization from invitation
            )
            user.set_password(form.password.data)
